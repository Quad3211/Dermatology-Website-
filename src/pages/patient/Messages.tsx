...
export default function PatientMessages() {
  ...
  const filtered = consultations.filter((c) =>
    (c.doctor?.full_name || "Dermatologist")
      .toLowerCase()
  ...
  const doctorLabel = (c: Consultation) =>
    c.doctor?.full_name
      ? `Dr. ${c.doctor.full_name.replace(/^Dr\.\s*/i, "")}`
      : "Specialist";

  const handleJoinCall = (consult: Consultation) => {
    setCallConsultId(consult.id);
    setCallDoctorName(doctorLabel(consult));
  };
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-white/10">
            <div className="flex items-center gap-3">
              ...
              <span className="font-semibold text-white">
                Video Call with {callDoctorName}
              </span>
            </div>
            <button
            ...
            filtered.map((c) => (
              ...
              >
                ...
                <p className="text-xs text-slate-500 line-clamp-1">
                  {c.last_message?.content || "Starting consultation..."}
                </p>
                {selectedId === c.id && (
        ...
        {selectedConsultation ? (
          <div className="h-full flex flex-col">
            ...
            <div className="flex items-center justify-between px-5 py-3 border-b border-slate-100 bg-white shrink-0">
              <span className="font-semibold text-slate-700">
                {doctorLabel(selectedConsultation)}
              </span>
              {/* Only show call button if a doctor is assigned */}
              {selectedConsultation.doctor_id && (
                <button
            ...
            <div className="flex-1 overflow-hidden">
              <SecureTextChat
                ...
                role="patient"
                otherPartyName={doctorLabel(selectedConsultation)}
              />
            </div>
          </div>
        ) : (
          <div className="flex-1 flex flex-col items-center justify-center p-12 text-center text-slate-400">
            ...
            <p className="max-w-xs text-sm leading-relaxed">
              Select a consultation from the list to message your dermatologist
              directly and securely, or start a video call.
            </p>
          </div>
        )}
      </div>
